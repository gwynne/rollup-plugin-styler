...
const baseDir$1 = path.dirname(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href))));
...
function loadModule (moduleId) {
  ...
  try {
    loaded[moduleId] = require$1(resolveSync([moduleId, `./${moduleId}`], options));
  } catch {
...
function ensureUseOption(opts) {
  ...
  if (opts.use === undefined) return Object.values(all);else if (!Array.isArray(opts.use)) throw new TypeError("`use` option must be an array!");
...
function ensurePCSSPlugins(plugins) {
  if (plugins === undefined) return [];else if (typeof plugins !== "object") throw new TypeError("`plugins` option must be an array or an object!");
  const ps = [];
...
const resolve$2 = async (inputUrl, basedir, extensions) => {
  ...
  const {
    url
  } = qs.parseUrl(inputUrl, parseOptions);
    ...
    async Once(css, {
      ...
      css.walkDecls(decl => {
        ...
        walkUrls(parsed, (url, node) => {
          ...
          // Use PostCSS imports
          if (decl.source?.input.file && imported.includes(decl.source.input.file)) basedirs.add(path.dirname(decl.source.input.file));
          // Use SourceMap
  ...
  async process({
    ...
    if (options.inject) {
      if (typeof options.inject === "function") {
        ...
        if (!injectorId) {
          const opts = {
            basedirs: [path.join(baseDir, "runtime")]
          };
...
async function loadSass (impl) {
  ...
  if (impl) {
    return import(impl).then(provided => {
...
const loader$1 = {
  ...
  async process({
    ...
    const stylus = await import('stylus').then(m => m.default);
